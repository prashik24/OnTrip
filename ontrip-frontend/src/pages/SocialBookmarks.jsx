  ...
  return (
    <article className="socialCard socialPostCard">
        ...
        <div className={`socialMediaGrid ${post.media.length === 1 ? "single" : "double"}`}>
          {post.media.map((item, index) =>
            item.type === "video" ? (
              ...
              <img key={`${item.url}-${index}`} src={item.url} alt="bookmark media" className="socialMediaItem" />
            )
          )}
        </div>
      ) : null}
      ...
      <div className="socialActionBar">
        ...
        <div className="socialActionStatic">🔖 Saved</div>
      </div>
    </article>
  );
}
    ...
    async function init() {
      try {
        ...
        const [profileRes, postsRes] = await Promise.all([
          ...
          apiFetch("/api/social/posts/bookmarks/me"),
        ]);
      ...
      <div className="socialLayout">
        ...
        <main className="socialMain">
          <div className="socialCard socialSectionHeaderCard">
            ...
            <p>All posts you saved for later.</p>
          </div>

          {loading ? (
            <LoadingSpinner text="Loading bookmarks..." />
          ) : posts.length === 0 ? (
            <div className="socialCard socialEmptyState">No bookmarked posts yet.</div>
          ) : (
        ...
        <aside className="socialRightBar">
          <div className="socialCard socialTipsCard">
            ...
            <div className="socialTipsList">
              <div>Save useful posts for later reading.</div>
              <div>Bookmark trip ideas and guides.</div>
            </div>
          </div>
        </aside>
      </div>
